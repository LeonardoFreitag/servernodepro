...
function getFee(req, res, next) {
  const lat = parseFloat(req.query.lat);
  const lng = parseFloat(req.query.lng);
  if (isNaN(lat) || isNaN(lng)) {
    res.status(400).json({
      error: 'invalid_params',
      message: 'Parâmetros lat e lng são obrigatórios e devem ser números válidos.'
    });
    return;
  }
    ...
    db.query('SELECT ORIGIN_LAT, ORIGIN_LNG FROM CONFIG', (err, configRows) => {
      ...
      if (isNaN(originLat) || isNaN(originLng)) {
        ...
        res.status(500).json({
          ...
          message: 'Coordenadas de origem do estabelecimento não configuradas (ORIGIN_LAT / ORIGIN_LNG).'
        });
        return;
      }
      const distanceKm = parseFloat(haversineKm(originLat, originLng, lat, lng).toFixed(2));
      db.query('SELECT FIRST 1 ID, DIST_MIN, DIST_MAX, FEE FROM DELIVERY_ZONES WHERE ? >= DIST_MIN AND ? < DIST_MAX AND ACTIVE = ?', [distanceKm, distanceKm, 'S'], (err, zoneRows) => {
        db.detach();
        if (err) {
          next(err);
          return;
  ...
  _nodeFirebird.default.attach(_firebird.default, (err, db) => {
    ...
    db.query('SELECT WEB_KEY, ORIGIN_LAT, ORIGIN_LNG FROM CONFIG', (err, configRows) => {
      ...
      if (isNaN(originLat) || isNaN(originLng)) {
        ...
        res.status(500).json({
          error: 'config_missing',
          message: 'Coordenadas de origem não configuradas.'
        });
        return;
      }
      db.query("SELECT DIST_MIN, DIST_MAX, FEE FROM DELIVERY_ZONES WHERE ACTIVE = 'S' ORDER BY DIST_MIN", [], (err, zoneRows) => {
        db.detach();
        if (err) {
          next(err);
          return;
        ...
        fb.firestore().collection('deliveryConfig').doc(idProvider).set(payload, {
          merge: true
        }).then(() => {
          res.status(200).json({
            success: true,
