...
const saveCustomer = customer => {
  ...
  const {
    ...
    whats,
    comments
  } = customer; // console.log(data);

  Firebird.attach(options, function (err, db) {
    if (err) throw err; // db = DATABASE

    db.query('SELECT oretorno FROM WEB_CLIENTES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [data.id, data.email, data.password, data.name, data.cpf, data.zipcode, data.address, data.number, data.neigh, data.feeDelivery, data.complement, data.city, data.uf, data.ibge, data.whats, data.comments], function (err, result) {
      if (err) {
